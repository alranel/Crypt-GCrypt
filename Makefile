# This Makefile is for the Crypt::GCrypt extension to perl.
#
...
#     ABSTRACT => q[Perl interface to the GNU libgcrypt library]
#     AUTHOR => q[Alessandro Ranellucci <aar@cpan.org>]
#     DEFINE => q[]
#     LIBS => q[-L-lgcrypt -lgcrypt]
#     NAME => q[Crypt::GCrypt]
#     VERSION_FROM => q[GCrypt.pm]
...
TO_INST_PM = GCrypt.pm

PM_TO_BLIB = GCrypt.pm \
	$(INST_LIB)/Crypt/GCrypt.pm
...
XSUBPPDIR = /System/Library/Perl/5.8.6/ExtUtils
XSUBPP = $(XSUBPPDIR)/xsubpp
XSPROTOARG = 
XSUBPPDEPS = /System/Library/Perl/5.8.6/ExtUtils/typemap typemap $(XSUBPP)
XSUBPPARGS = -typemap /System/Library/Perl/5.8.6/ExtUtils/typemap -typemap typemap
XSUBPP_EXTRA_ARGS = 
...
# Crypt::GCrypt might depend on some other libraries:
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW)\
	  GCrypt.pm $(INST_MAN3DIR)/Crypt::GCrypt.$(MAN3EXT) 
...
clean :: clean_subdirs
	-$(RM_RF) GCrypt.c ./blib $(MAKE_APERL_FILE) $(INST_ARCHAUTODIR)/extralibs.all $(INST_ARCHAUTODIR)/extralibs.ld perlmain.c tmon.out mon.out so_locations pm_to_blib *$(OBJ_EXT) *$(LIB_EXT) perl.exe perl perl$(EXE_EXT) $(BOOTSTRAP) $(BASEEXT).bso $(BASEEXT).def lib$(BASEEXT).def $(BASEEXT).exp $(BASEEXT).x core core.*perl.*.? *perl.core core.[0-9] core.[0-9][0-9] core.[0-9][0-9][0-9] core.[0-9][0-9][0-9][0-9] core.[0-9][0-9][0-9][0-9][0-9]
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_F) $(INST_STATIC)
	$(RM_F)  $(INST_LIB)/Crypt/GCrypt.pm $(MAKEFILE_OLD) $(FIRST_MAKEFILE)
...
metafile :
	...
	$(NOECHO) $(ECHO) 'name:         Crypt-GCrypt' >> META.yml
	$(NOECHO) $(ECHO) 'version:      1.16' >> META.yml
	$(NOECHO) $(ECHO) 'version_from: GCrypt.pm' >> META.yml
...
ppd:
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="1,16,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perl interface to the GNU libgcrypt library</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Alessandro Ranellucci &lt;aar@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	$(NOECHO) $(PERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')'\
	  GCrypt.pm $(INST_LIB)/Crypt/GCrypt.pm 
	$(NOECHO) $(TOUCH) $@
